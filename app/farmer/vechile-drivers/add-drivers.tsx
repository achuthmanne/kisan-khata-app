...
<AppText style={{ fontSize: 13, color: "#92400E", lineHeight: 18 }} language={language}>
                {language === "te" 
                  ? `మీరు పాత సాగు సంవత్సరం (${activeSession}) లో డ్రైవర్ని నమోదు చేస్తున్నారు.` 
                  : `You are adding a driver to an older season (${activeSession}).`}
              </AppText>
            </View>
          </View>
        )}
      ...
      <Modal visible={showDuplicateModal} transparent animationType="fade" statusBarTranslucent>
        <View style={styles.modalOverlayStandard}>
          <View style={styles.modalContentStandard}>
            ...
            <AppText style={styles.modalSubStandard} language={language}>
              {language === "te" ? "ఈ ఫోన్ నంబర్ తో డ్రైవర్ వివరాలు ఇప్పటికే ఉన్నాయి.\n\nమీరు ఖచ్చితంగా మళ్లీ జతచేయాలనుకుంటున్నారా?" : "A driver with this phone number already exists.\n\nAre you sure you want to add this duplicate entry?"}
            </AppText>
            <View style={styles.modalButtonsStandard}>
